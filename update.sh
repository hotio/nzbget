...
if [[ ${1} == "screenshot" ]]; then
    SERVICE_IP="http://nzbget:tegbzn6789@$(ping -c1 -4 service | sed -nE 's/^PING[^(]+\(([^)]+)\).*/\1/p'):6789"
    NETWORK_IDLE="2"
...
EOF
    ...
    [[ -z ${version} ]] && exit 1
    sed -i "s/{NZBGET_VERSION=[^}]*}/{NZBGET_VERSION=${version}}/g" .drone.yml
    sed -i "s/{NZBGET_VERSION_SHORT=[^}]*}/{NZBGET_VERSION_SHORT=${version//-testing/}}/g" .drone.yml
